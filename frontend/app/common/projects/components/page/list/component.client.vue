...
<template>
  <div class="grid gap-4">
    <div class="flex items-center justify-between">
      <div>
        <h2 class="text-base font-semibold text-slate-100">Проекты</h2>
        <p class="text-sm text-slate-400">Группируйте приложения по проектам</p>
      </div>
    ...
    <AppCard v-if="showForm">
      <div class="grid gap-3">
        <h3 class="text-sm font-medium text-slate-200">Создать проект</h3>
        <AppInput v-model="form.name" label="Название" placeholder="billing-core" />
        <AppInput v-model="form.description" label="Описание (опционально)" placeholder="Биллинговые сервисы" />
    ...
    <div v-if="!projects?.length" class="rounded-xl border border-white/5 bg-slate-900/40 px-5 py-8 text-center text-sm text-slate-400">
      Нет проектов. Создайте первый.
    </div>

    <AppCard v-for="p in projects" :key="p.id" class="cursor-pointer hover:border-sky-400/20" @click="router.push(`/projects/${p.id}`)">
      <div class="flex items-center justify-between gap-4">
        <div class="min-w-0">
          <p class="truncate font-medium text-slate-100">{{ p.name }}</p>
          <p class="truncate text-xs text-slate-400">{{ p.description || p.slug }}</p>
        </div>
        <div class="flex shrink-0 items-center gap-2">
          <AppBadge :label="`${p.app_count ?? 0} приложений`" tone="muted" />
          <button class="p-1 text-xs text-slate-500 hover:text-rose-400" @click.stop="removeProject(p.id)">✕</button>
        </div>
      </div>
    </AppCard>
  </div>
